...
class RoadMap:
    ...
    def path_cost(self, start, road_list):
        ...
        for x in road_list:
            all_streets = self.get_result(current_place)
            if x in all_streets:
                last_place = current_place
                current_place = self.get_result(current_place, x)
                cost += self.get(current_place, last_place)
            else:
                return 0.0
        return cost